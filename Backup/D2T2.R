...
dataClimates <- read.table(file="Climate2.csv", sep=",", header=TRUE)
...
RainSpellMessage<-function(LM){
	...
	n<-length(x);i<-1;j<-0;k<-1;l<-0
	highest_sequence<-matrix(c(0,0,0,0,0,0))
	hs_data<-list()
	hs_data[[10]]<-1
	hs_data[[10]][2]<-1
	...
	status<-0;
	for(i in i:n){
		if((LM[i,"Rainfall"]==0)&&(status==2)){
			hs_data[[k]][2]<-as.character(LM[i-1,"Date"])
			k<-k+1
		}
		if(LM[i,"Rainfall"]==0){
			highest_sequence[j] <- highest_sequence[j] + amt
			# print(amt)
			status=0
			# print("asd")
			amt<-0
		}
		if((LM[i,"Rainfall"]>0)&&(status==0)){
			status=1
			amt<-amt+1
			hs_data[[k]][1]<-as.character(LM[i,"Date"])
			j<-j+1
			# print("xxx")
		}
		if((LM[i,"Rainfall"]>0)&&(status==1)){
			status=2
			amt<-amt+1
			# print("yyy")
		}
	...
	total_days<-max(highest_sequence)
	highest_index<-which.max(highest_sequence)
	# hs_data<-hs_data[[highest_index]]
	interaval_data <- hs_data[[highest_index]]
	begin_day<-substrRight(hs_data[[1]][1],2)
	end_day<-substrRight(hs_data[[1]][2],2)
	oi_begin<-ordinal_indicator(begin_day)
	oi_end<-ordinal_indicator(end_day)
	print(begin_day)
	print(end_day)

	msg<-paste(total_days,"days from",begin_day,oi_begin,"to",end_day,oi_end)
	return(msg)
}
...
AQ_Intro <- function(){
	intro_AQ <- as.matrix(read.table(file="AQ_intro_phrase.csv", sep=",", header=TRUE))
...
Prediction_Intro <- function(){
		...
		if(x==3){
			return("Tommorow is forecasted that the sky will be")
		}
		if(x==4){
...
change_word_bank_AQ <- function (fragmentCode){
	phraseAQ <- read.table(file="AQ_phrase_bank.csv", sep=",", header=TRUE)
...
MonthlyMsg3_aggregation<-function(msg2){
	msg<-paste(msg2,".")
	return(msg)
}
MonthlyMsg3 <- MonthlyMsg3_aggregation(RainExtremeMsg)
...
MonthlyMsg<- paste(MonthlyMsg1,MonthlyMsg2,MonthlyMsg3,MonthlyMsg4,MonthlyMsg5)
#-----------------------------------------------------------------------------------------------------------#
...
all_result<-paste(Prediction_Result,"Regarding to monthly summary result,",MonthlyMsg)
