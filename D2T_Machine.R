...
setwd("C:/Users/Lyra/Desktop/D2T_Apps")
...
if(dataCli[ncol(dataCli),"Rainfall"]==9999||dataCli[ncol(dataCli),"Rainfall"]==8888){
#Rainfall Prediction based on cloud, temperature, and wind
datasetRainfall <- data.frame(c(dataCli["Cloud.Coverage"],
								dataCli["Average.Temperature"],
								dataCli["Wind.Speed"],
								dataCli["Rainfall"]) #output
								)

## Predict Using GD Model
## get z-factor Data
dataSet <- datasetRainfall
...
i1<-dataCli[ncol(dataCli),"Cloud.Coverage"]
i2<-dataCli[ncol(dataCli),"Average.Temperature"]
i3<-dataCli[ncol(dataCli),"Wind.Speed"]
dataTestInput <-  data.frame(i1,i2,i3)
...
RainSpellMessage<-function(LM){
	...
	in_spell<- as.integer(substrRight(seq[max_index,2],2))	
	out_spell<-as.integer(substrRight(seq[max_index,3],2)) 
	in_mnth<-substr(seq[max_index,2],6,7)
	out_mnth<-substr(seq[max_index,3],6,7)
	...
	if(in_mnth==out_mnth){
		...
		max_spell<-paste(max(seq[,1])," days from ",in_spell,in_ordinal, " ", in_mnth,
					 " to ",out_spell, out_ordinal, " ", out_mnth," ",sep="")
...
Temperature_State<-paste(TrendDesc_Temperature, InterpretationResult_temperature)
